...
func CreateSummaryCqs(dbName string) ([]*libcq.ContinuousQuery, error) {
	...
	volCqs, err := libcq.NewContinuousQuery(
		...
		"SELECT SUM(eth_amount) AS total_eth_volume, SUM(usd_amount) AS total_usd_amount, COUNT(eth_amount) AS total_trade, "+
			"MEAN(usd_amount) AS usd_per_trade, MEAN(eth_amount) AS eth_per_trade INTO trade_summary "+
	...
	totalBurnFeeCqs, err := libcq.NewContinuousQuery(
		...
		dayResampleInterval,
		dayResampleFor,
		"SELECT SUM(src_burn_fee)+SUM(dst_burn_fee) AS total_burn_fee INTO burn_fee_summary FROM trades",
		"1d",
		supportedTimeZone(),
	)
	if err != nil {
