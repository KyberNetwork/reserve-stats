...
// The query is:
// SELECT time, reserve_addr, amount, log_index FROM burn_fees WHERE_clause GROUP BY block_number, trade_log_index
func (is *InfluxStorage) rowToBurnFees(row models.Row) (uint, uint, []common.BurnFee, error) {
	var (
		burnFees      []common.BurnFee
		blockNumber   uint64
		tradeLogIndex uint64
	)
	blockNumber, err := influxdb.GetUint64FromTagValue(row.Tags["block_number"])
	if err != nil {
		return uint(blockNumber), uint(tradeLogIndex), nil, err
	}
	...
	return uint(blockNumber), uint(tradeLogIndex), burnFees, nil
}
...
func (is *InfluxStorage) rowToWalletFees(row models.Row) (uint, uint, []common.WalletFee, error) {
	var (
	...
	for _, value := range row.Values {
		...
		// is.sugar.Debugf("Wallet FeeBlock Number %d, Log_index %d, tx_hash %s", blockNumber, tradeLogIndex, txHash.String())
	...
	return uint(blockNumber), uint(tradeLogIndex), walletFees, nil
}
...
func (is *InfluxStorage) rowToTradeLog(row models.Row,
	burnFeesByIndex map[uint]map[uint][]common.BurnFee,
	walletFeesByIndex map[uint]map[uint][]common.WalletFee) (common.TradeLog, error) {
	...
	ethReceivalAddr, err := influxdb.GetAddressFromInterface(value[1])
	if err != nil {
		return tradeLog, fmt.Errorf("failed to get eth_receival_addr: %s", err)
	}

	humanizedEthReceival, err := influxdb.GetFloat64FromInterface(value[2])
	if err != nil {
	...
	userAddr, err := influxdb.GetAddressFromInterface(value[3])
	if err != nil {
		return tradeLog, fmt.Errorf("failed to get user_addr: %s", err)
	}

	srcAddress, err := influxdb.GetAddressFromInterface(value[4])
	if err != nil {
		return tradeLog, fmt.Errorf("failed to get src_addr: %s", err)
	}

	humanizedSrcAmount, err := influxdb.GetFloat64FromInterface(value[6])
	if err != nil {
	...
	dstAddress, err := influxdb.GetAddressFromInterface(value[5])
	if err != nil {
		return tradeLog, fmt.Errorf("failed to get dst_addr: %s", err)
	}

	humanizedDstAmount, err := influxdb.GetFloat64FromInterface(value[7])
	if err != nil {
	...
	fiatAmount, err := influxdb.GetFloat64FromInterface(value[8])
	if err != nil {
	...
	txHash, err := influxdb.GetTxHashFromInterface(value[12])
	if err != nil {
		return tradeLog, err
	}
	burnFees := burnFeesByIndex[uint(blockNumber)][uint(logIndex)]
	if burnFees == nil {
		burnFees = []common.BurnFee{}
	}

	walletFees := walletFeesByIndex[uint(blockNumber)][uint(logIndex)]
	if walletFees == nil {
		walletFees = []common.WalletFee{}
	}
